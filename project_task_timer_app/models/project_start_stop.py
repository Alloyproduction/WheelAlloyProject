...
class Inherit_project_task(models.Model):
	...
	@api.multi
	@api.onchange('date_task_deadline')
	def _set_deadline(self):

		 # self.date_deadline =self.date_task_deadline.date()
		 for x in self.duration_ids:
			 if self.date_task_deadline :
				 x.date_deadline = self.date_task_deadline
				 print(x.date_deadline)
				 x.write({'date_deadline':self.date_task_deadline})
		 # self.write({'date_deadline':self.date_task_deadline.date()})
	...
	def start_play(self):
		...
		for taskes in self:
			timeline.create({
				...
				'date_deadline':self.date_task_deadline,
			})
...
class project_duration_calculation(models.Model):
	...
	Eduration = fields.Float('EDuration', compute='_compute_durationE', store=True)
	...
	def _compute_duration(self):
		for blocktime in self:
			if blocktime.date_end:
				...
				diff = d2 - d1
				blocktime.duration = round(diff.total_seconds() / (60.0*60.0*1000), 2) + 0.01
			# else:
			# 	blocktime.duration = 0.0

	@api.depends('date_deadline' )
	def _compute_durationE(self):

		for blocktime in self:
			if blocktime.date_deadline:
				d1 =  fields.Datetime.now()
				d2 = fields.Datetime.from_string(blocktime.date_deadline)
				diff = d2 - d1
				blocktime.Eduration = round(diff.total_seconds() / (60.0 * 60.0 * 1000), 2) + 0.01
