...
class AccountFollowupReport(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        ...
        if self.env.context.get('print_mode'):
            headers = [{},
                   {'name': _('Date'), 'class': 'date', 'style': 'text-align:center; white-space:nowrap; font-size:11px;'},
                   ...
                   {'name': _('Due Date'), 'class': 'date', 'style': 'text-align:center; white-space:nowrap;font-size:11px;'},
                   ...
                   {'name': _('Total Due'), 'class': 'number o_price_total', 'style': 'text-align:right; white-space:nowrap;font-size:11px;'},
                   ]
        return headers
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                ...
                if is_overdue:
                    date_due = {'name': date_due, 'class': 'color-red date', 'style': 'white-space:nowrap;text-align:center;color: red;font-size:11px;'}
                if is_payment:
                    date_due = ''
                move_line_name = aml.invoice_id.name or aml.name
                if self.env.context.get('print_mode'):
                    move_line_name = {'name': move_line_name, 'style': 'text-align:right; white-space:normal;font-size:11px;'}
            ...
            lines.append({
                ...
                'unfoldable': False,
                'level': 0,
                'columns': [{'name': v,'style': 'text-align:right;white-space:normal; font-size:11px;',} for v in [''] * (7 if self.env.context.get('print_mode') else 5) + [total >= 0 and _('Total Due') or '', total_due]],
            })
            if total_issued > 0:
                ...
                lines.append({
                    ...
                    'unfoldable': False,
                    'level': 0,
                    'columns': [{'name': v,'style': 'text-align:right;white-space:normal; font-size:11px;',} for v in [''] * (7 if self.env.context.get('print_mode') else 5) + [_('Total Overdue'), total_issued]],
                })
