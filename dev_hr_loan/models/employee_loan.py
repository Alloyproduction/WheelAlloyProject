...
class employee_loan(models.Model):
    ...
    loan_state = [('draft', 'Draft'), ('request', 'Submit Request'), ('dep_approval', 'Department Approval'),
                  ('hr_approval', 'HR Approval'), ('paid', 'Paid'), ('done', 'Done'), ('close', 'Close'),
    ...
    email_for_manager = fields.Char(string='Manager Email', related='manager_id.work_email', readonly=True,)
    ...
    def _check_loan(self):
        ...
        loan_ids = self.search([('employee_id', '=', self.employee_id.id), ('date', '<=', e_date),
                                ('date', '>=', s_date)])
        loan = len(loan_ids)
    ...
    def onchange_employee_id(self):
        if self.employee_id:
            ...
            # self.email_for_manager = self.manager_id.work_email
            # print('email m =', self.manager_id.work_email)
    ...
    def action_send_request(self):
        ...
        # print('sending email')
        template_id = self.env.ref('dev_hr_loan.dev_dep_manager_request').id
        # print('template_idddd =', template_id)
        ...
        #     template_id = ir_model_data.get_object_reference('dev_hr_loan', 'dev_dep_manager_request')
    ...
    # change Account payable to credit
    def paid_loan(self):
        ...
        if acc_move_id:
            ...
            # print('debit 1=', self.loan_amount)
            lst.append(val)
            if self.interest_amount:
                val = (0,0,{
                                ...
                                # 'debit':self.interest_amount or 0.0,
                                'move_id': acc_move_id.id,
                            })
                # print('credit 1=', self.interest_amount)
                lst.append(val)
            ...
            val = (0,0,{
                            ...
                            'account_id': self.loan_type_id and self.loan_type_id.loan_account_payable.id,
                            'partner_id': self.employee_id.address_home_id and self.employee_id.address_home_id.id or False,
                            'name': '/',
