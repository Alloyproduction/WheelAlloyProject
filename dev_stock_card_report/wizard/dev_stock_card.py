...
class dev_stock_card(models.TransientModel):
    ...
    Emp_request_show = fields.Boolean('Show only Employee Request')
    ...
    def in_lines(self, product_ids):
        ...
        query = """select DATE(sm.date) as date, sm.origin as origin, sm.reference as ref, pt.name as product,\
                  sm.product_uom_qty as in_qty, sm.product_uom as m_uom,pt.uom_po_id as p_uom, pp.id as product_id , sm.employee_user_id_move as emp_request from stock_move as sm \
                  ...
                  """
        if self.Emp_request_show:
            query += " and sm.employee_user_id_move is not null "
    ...
    def out_lines(self, product_ids):
        ...
        query = """select DATE(sm.date) as date, sm.origin as origin, sm.reference as ref, pt.name as product,\
                      sm.product_uom_qty as out_qty,sm.product_uom as m_uom,pt.uom_id as p_uom, pp.id as product_id ,sm.employee_user_id_move as emp_request \
    ...
    def create_excel_header(self, worksheet, main_header_style, text_left, text_center, left_header_style, text_right,
                            header_style):
        worksheet.write_merge(0, 1, 1, 3, 'Stock Card Report', main_header_style)
        row = 2
        col = 1
        ...
        worksheet.write(row, 5, 'Employee Request', header_style)
        ...
        for line in lines:
            ...
            for val in line.get('values'):
                ...
                emp_request = val.get('emp_request')
                emp = self.env['res.users'].search([('id', '=', emp_request)])
                ...
                worksheet.write(row, 5, emp.name, text_center)
                row += 1
