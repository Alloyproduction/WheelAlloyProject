...
class ReportForAbsencecustom(models.TransientModel):
    _name = "report.absence.custom"
    ...
    def get_report_all(self):
        ...
        print('data==', data)
        return self.env.ref('bi_hr_overtime_request.absence_report').report_action(self, data=data)


class AbsenceReportView(models.AbstractModel):
    _name = "report.bi_hr_overtime_request.absence_cu"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data['form']['employee_for_all']:
            ...
            ove_id = self.env['absence.monthly'].search([('employee_id', '=', data['form']['employee_for_all'])])
            print('MM=', ove_id)
            for rec in ove_id:
                docs.append({
        ...
        if data['form']['date_from'] or data['form']['date_to']:
            ...
            ourloans = self.env['absence.monthly'].search(domains, order='start_date asc')
            print('MM=', ourloans)
            for rec in ourloans:
                docs.append({
                    'employee_name': rec.employee_id.name,
